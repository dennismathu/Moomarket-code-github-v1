        ...
        <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                        {currentUser ? (
                            <div className="flex items-center gap-4">
                                ...
                                <Link
                                    to={currentUser.role === 'admin' ? '/admin' : currentUser.role === 'seller' ? '/dashboard/seller' : '/dashboard/buyer'}
                            ...
                            <button className="bg-emerald-600 text-white px-5 py-2 rounded-lg font-medium hover:bg-emerald-700 transition-colors shadow-sm">
                                Log In
                            </button>
                        )}
                    </div>
            ...
            {isMobileMenuOpen && (
                <div className="md:hidden bg-white border-t border-slate-100 px-4 pt-2 pb-6 space-y-1 shadow-lg">
                    ...
                    {currentUser && (
                        <Link to={currentUser.role === 'seller' ? '/dashboard/seller' : '/dashboard/buyer'} className="block px-3 py-4 text-base font-bold text-emerald-600" onClick={toggleMobileMenu}>
                            <div className="flex items-center gap-2">
                                <LayoutDashboard size={18} />
                                Go to My Dashboard
                            </div>
